...
MODEL_SAVE_PATH = OUTPUT_DIR / "models" / "best_model.pth"
HISTORY_SAVE_PATH = OUTPUT_DIR / "logs" / "training_history.csv"

IMAGE_SIZE = 256
BATCH_SIZE = 32
EPOCHS = 50  # Increased from 10 for better convergence
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a binary CNN on the processed AI Image Detector dataset.")
    parser.add_argument("--data-root", type=Path, default=DATASET_ROOT, help="Path to dataset/processed.")
    ...
    parser.add_argument("--val-fraction", type=float, default=VAL_FRACTION, help="Validation split fraction taken from the training set.")
    parser.add_argument("--seed", type=int, default=RANDOM_SEED, help="Random seed for reproducible splitting.")
...
def save_history(rows: list[dict[str, float]]) -> None:
    with HISTORY_SAVE_PATH.open("w", newline="", encoding="utf-8") as file:
...
def main() -> None:
    ...
    split_data, class_weights = load_datasets(args.data_root, args.seed, args.val_fraction)
    ...
    print("Classes:", split_data.class_names)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"Train Loss: {train_scores['loss']:.4f} | Train Acc: {train_scores['accuracy']:.4f} | "
            f"Val Loss: {val_scores['loss']:.4f} | Val Acc: {val_scores['accuracy']:.4f}"
        )
        print(
            f"Val Precision: {val_scores['precision']:.4f} | Val Recall: {val_scores['recall']:.4f} | Val F1: {val_scores['f1']:.4f}"
        )
        print("-" * 60)
