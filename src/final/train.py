...
MODEL_PATH = OUTPUT_DIR / "models" / "model_final.pth"
THRESHOLD_PATH = OUTPUT_DIR / "models" / "model_final_threshold.json"
RUN_TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M")
LOG_PATH = OUTPUT_DIR / "logs" / f"training_log_final_{RUN_TIMESTAMP}.csv"
PLOT_PATH = OUTPUT_DIR / "plots" / f"training_overfit_final_{RUN_TIMESTAMP}.png"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train final gated multi-evidence CNN with auxiliary branch losses.")
...
def save_training_plots(rows: list[dict[str, float]], plot_path: Path, start_time: float) -> None:
    ...
    fig.suptitle("Final Training Diagnostics (Overfitting / Underfitting)")
...
def train_one_epoch(
    ...
    for batch_index, (images, labels) in enumerate(loader, start=1):
        ...
        loss = compute_final_loss(output, labels, criterion, rgb_loss_weight, residual_loss_weight)
        loss.backward()
        optimizer.step()
        if ema is not None:
            ema.update(model)
...
def main() -> None:
    ...
    train_loader = build_balanced_train_loader(
    ...
    log_message(start_time, "Train sampler: class-balanced weighted sampling")
...
# python .\src\final\train.py --epochs 50 --focal-gamma 1.0 --focal-alpha 0.5 --ema-decay 0.995 --min-lr-ratio 0.2
