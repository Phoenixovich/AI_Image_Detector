...
class TrainTransform:
    """Artifact-preserving augmentation: light codec/resample jitter without washing out traces."""
    ...
    def __call__(self, image: Image.Image) -> torch.Tensor:
        ...
        if self.rng.random() < 0.28:
            quality = self.rng.randint(68, 96)
        ...
        if self.rng.random() < 0.12:
            image = image.filter(ImageFilter.GaussianBlur(radius=self.rng.uniform(0.15, 0.55)))

        if self.rng.random() < 0.14:
            image = image.filter(ImageFilter.UnsharpMask(radius=1.0, percent=90, threshold=2))

        if self.rng.random() < 0.22:
            image = ImageEnhance.Brightness(image).enhance(self.rng.uniform(0.96, 1.04))
        if self.rng.random() < 0.22:
            image = ImageEnhance.Contrast(image).enhance(self.rng.uniform(0.96, 1.05))
        if self.rng.random() < 0.16:
            image = ImageEnhance.Color(image).enhance(self.rng.uniform(0.96, 1.05))
