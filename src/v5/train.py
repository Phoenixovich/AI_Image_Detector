...
MODEL_PATH = OUTPUT_DIR / "models" / "model5.pth"
THRESHOLD_PATH = OUTPUT_DIR / "models" / "model5_threshold.json"
RUN_TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M")
LOG_PATH = OUTPUT_DIR / "logs" / f"training_log_v5_{RUN_TIMESTAMP}.csv"
PLOT_PATH = OUTPUT_DIR / "plots" / f"training_overfit_v5_{RUN_TIMESTAMP}.png"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train v5 CNN with balanced sampling and threshold tuning.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Stop if val balanced accuracy does not improve for this many epochs.",
    )
    parser.add_argument(
        "--early-stop-min-delta",
        type=float,
        default=0.001,
        help="Minimum val balanced accuracy improvement required to reset patience.",
    )
    parser.add_argument(
        "--hard-examples-dir",
        type=Path,
...
def save_training_plots(rows: list[dict[str, float]], plot_path: Path) -> None:
    if not rows:
        return
    ...
    try:
        ...
        print(f"Skipping training plots (matplotlib unavailable): {exc}")
        return
    ...
    fig.suptitle("V5 Training Diagnostics (Overfitting / Underfitting)")
...
def mine_hard_examples_from_train(
    ...
    print(
        f"  Hard mining batches: {total_batches} | workers: {max(0, int(num_workers))}",
        flush=True,
    )
    ...
    with torch.no_grad():
        for batch_index, (images, labels, names) in enumerate(eval_loader, start=1):
            ...
            if batch_index == 1 or batch_index % max(1, log_interval) == 0 or batch_index == total_batches:
                progress_pct = 100.0 * batch_index / max(1, total_batches)
                print(f"  Hard batch {batch_index}/{total_batches} ({progress_pct:5.1f}%)", flush=True)
...
def main() -> None:
    ...
    print(f"Hard examples found from inputs (by name): {len(static_hard_example_names)}")
    print(f"Dynamic hard mining enabled: {args.dynamic_hard_mining}")
    print(f"Dynamic hard mining threshold: {args.dynamic_hard_threshold:.2f}")
    print(f"Dynamic hard mining workers: {max(0, int(args.hard_mining_workers))}")
    print(f"Hard examples present in train split: {train_hard_count}")
    print(f"Hard sampling multiplier: {effective_multiplier:.2f}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.dynamic_hard_mining:
            print("Starting dynamic hard example mining on train split...", flush=True)
            ...
            print(f"Dynamic hard examples mined from train split: {len(dynamic_hard_example_names)}")
        ...
        improvement = val_scores["balanced_accuracy"] - best_val_balanced_accuracy
        ...
        if epochs_without_improvement >= args.early_stop_patience:
            print(
                ...
                f"no val_balanced_accuracy improvement > {args.early_stop_min_delta:.4f} "
                f"for {args.early_stop_patience} epochs."
            )
            break

    save_logs(log_rows)
    save_training_plots(log_rows, PLOT_PATH)
