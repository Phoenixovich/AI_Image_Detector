...
MODEL_PATH = OUTPUT_DIR / "models" / "model6.pth"
THRESHOLD_PATH = OUTPUT_DIR / "models" / "model6_threshold.json"
RUN_TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M")
LOG_PATH = OUTPUT_DIR / "logs" / f"training_log_v6_{RUN_TIMESTAMP}.csv"
PLOT_PATH = OUTPUT_DIR / "plots" / f"training_overfit_v6_{RUN_TIMESTAMP}.png"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train v6 CNN with residual+SE blocks, focal loss, EMA and cosine schedule.")
    ...
    parser.add_argument("--focal-alpha", type=float, default=0.25, help="Focal loss alpha parameter for positive class balancing.")
...
def save_training_plots(rows: list[dict[str, float]], plot_path: Path, start_time: float) -> None:
    ...
    fig.suptitle("V6 Training Diagnostics (Overfitting / Underfitting)")
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_scores = train_one_epoch(model, train_loader, criterion, optimizer, DEVICE, args.log_interval, start_time, ema=ema)

        ema.apply(model)
        val_scores, val_threshold = evaluate_with_threshold_search(model, val_loader, criterion, DEVICE, args.log_interval, start_time)
        ema.restore(model)
    ...
    log_message(start_time, f"Training finished. Best val balanced accuracy: {best_val_balanced_accuracy:.4f}")
...
# python .\src\v6\train.py --epochs 50 --hard-weight-multiplier 1.0 --focal-gamma 1.0 --focal-alpha 0.5 --ema-decay 0.995 --min-lr-ratio 0.2