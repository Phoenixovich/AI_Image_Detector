...
MODEL_PATH = OUTPUT_DIR / "models" / "model7.pth"
THRESHOLD_PATH = OUTPUT_DIR / "models" / "model7_threshold.json"
RUN_TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M")
LOG_PATH = OUTPUT_DIR / "logs" / f"training_log_v7_{RUN_TIMESTAMP}.csv"
PLOT_PATH = OUTPUT_DIR / "plots" / f"training_overfit_v7_{RUN_TIMESTAMP}.png"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train v7 multi-evidence CNN with RGB and high-pass residual branches.")
...
def save_training_plots(rows: list[dict[str, float]], plot_path: Path, start_time: float) -> None:
    ...
    fig.suptitle("V7 Training Diagnostics (Overfitting / Underfitting)")
...
# python .\src\v7\train.py --epochs 50 --hard-weight-multiplier 1.0 --focal-gamma 1.0 --focal-alpha 0.5 --ema-decay 0.995 --min-lr-ratio 0.2
